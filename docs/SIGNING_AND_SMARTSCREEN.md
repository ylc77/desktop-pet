...
当前 `0.1.2-beta.3` 候选尚未配置 Windows Authenticode；最终 NSIS 安装包必须重新检查，预期 `Get-AuthenticodeSignature` 仍返回 `NotSigned`。内部测试人员必须先使用候选发布目录中的 `SHA256SUMS.txt` 核对：

```powershell
Get-FileHash '.\release\updater\0.1.2-beta.3\qijiang-desktop-pet_0.1.2-beta.3_x64-setup.exe' -Algorithm SHA256
Get-AuthenticodeSignature '.\release\updater\0.1.2-beta.3\qijiang-desktop-pet_0.1.2-beta.3_x64-setup.exe'
```
