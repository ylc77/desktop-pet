...
- 外观中心统一展示内置角色和本地导入角色，可切换、导入或删除非当前的本地角色包
- 角色级默认缩放、交互动作映射和归一化矩形点击区域
- 本地设置保存、开机启动、日志和开发者面板
- 关于页面、手动检查更新、自动检查开关、脱敏诊断导出和二次确认的默认设置恢复
...
Windows 更新安装前会严格保存设置、窗口状态和待确认目标版本；原生写入失败会阻止安装。正常路径由 Tauri Updater 启动 passive NSIS 后清理并退出旧进程，NSIS 完成后自动启动新版本，再由新进程确认实际版本并清理 pending 状态。生产元数据生成、发布准备、签名构建和 Gate 都要求用外部公钥真实验证安装包与 `.sig`，并绑定版本化文件名和实际字节数；这仍不能替代尚未完成的真实 A → B QA。
