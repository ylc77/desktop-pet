...
const failureText: Record<UpdaterFailureCategory, string> = {
  ...
  restartFailed: "更新安装流程已完成，但应用重新启动失败。重试不会再次安装。",
...
interface Props {
  ...
  onSkip: (version: string) => void | Promise<void>;
...
export function UpdatePanel({ store, skippedVersion, onSkip, onLater, onInstall }: Props) {
  ...
  const run = (operation: () => void | Promise<void>) => void Promise.resolve(operation()).catch(() => undefined);

  return (
    <section className="update-panel" aria-label="应用更新">
      <div className="panel-row">
        <strong>应用更新</strong>
        <button disabled={busy || !snapshot.configuration?.configured} onClick={() => run(() => store.check({ manual: true }))}>检查更新</button>
      </div>
      ...
      {update && ["available", "downloading", "readyToInstall", "installing", "restarting", "postponed", "skipped", "error"].includes(snapshot.status) && (
      ...
      {snapshot.status === "available" && update && (
        <div className="panel-actions">
          <button onClick={() => run(onInstall)}>立即更新</button>
          <button onClick={() => run(onLater)}>稍后提醒</button>
          <button onClick={() => run(() => onSkip(update.version))}>跳过此版本</button>
        </div>
      )}
      ...
      {snapshot.status === "readyToInstall" && (
        <div>
          ...
          <button onClick={() => run(onInstall)}>继续安装并重启</button>
        </div>
      )}
      {snapshot.status === "installing" && <p role="status">正在启动 Windows 被动安装流程…</p>}
      {snapshot.status === "restarting" && <p role="status">安装完成，正在重新启动七酱桌宠…</p>}
      {snapshot.status === "postponed" && <p role="status">已选择稍后提醒。本次待处理更新已安全释放。</p>}
      {snapshot.status === "skipped" && <p role="status">已跳过版本 {update?.version ?? ""}。以后仍可手动检查。</p>}
      ...
      {snapshot.status === "error" && snapshot.error && (
        <div className="update-error" role="alert">
          ...
          <button disabled={busy} onClick={() => run(() => store.retry())}>重试</button>
        </div>
      )}
    </section>
  );
