...
import { appSettingsPatchSchema, appSettingsSchema, DEFAULT_SETTINGS, resetSettingsPreservingCharacter } from "../src/core/settings/settingsSchema";
...
describe("settings schema", () => {
  ...
  it.each([null, "0.5", -1, Number.NaN])("rejects an invalid scale patch %s before it can be clamped", (scale) => {
    expect(appSettingsPatchSchema.safeParse({ scale }).success).toBe(false);
  });
  ...
  it.each([0.01, 0.1, 0.25, 2, 4])("preserves schema-compatible character scale %s", (scale) => {
